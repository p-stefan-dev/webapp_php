...
// Verificam daca un formular a fost trimis
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['save_title'])) {
        ...
        if (!empty($newTitle)) {
            try {
                $stmt = $pdo->prepare("UPDATE settings SET setting_value = ? WHERE setting_key = 'dashboard_title'");
                $stmt->execute([$newTitle]);
                $successMessage = 'Numele aplicației a fost actualizat!';
            } catch (PDOException $e) { $errorMessage = 'Eroare la salvarea numelui.'; }
        } else {
            $errorMessage = 'Numele aplicației nu poate fi gol.';
        }
    }

    // --- Procesare Imagine Login ---
        ...
        if (isset($_FILES['login_background']) && $_FILES['login_background']['error'] == 0) {
            ...
            if (!in_array($fileType, $allowedTypes)) {
                ...
                if (move_uploaded_file($_FILES["login_background"]["tmp_name"], $targetFile)) {
                    try {
                        $stmt = $pdo->prepare("UPDATE settings SET setting_value = ? WHERE setting_key = 'login_background_image'");
                        $stmt->execute([$targetFile]);
                        $successMessage = 'Imaginea de fundal a fost actualizată cu succes!';
                    } catch (PDOException $e) { $errorMessage = 'Eroare la salvarea căii imaginii în baza de date.'; }
                } else {
                    $errorMessage = 'Eroare la încărcarea fișierului.';
                }
            }
        } else {
            $errorMessage = 'Nu a fost selectat niciun fișier sau a apărut o eroare la încărcare.';
        }
    }

    // --- Procesare Adaugare Structura ---
    if (isset($_POST['add_structura'])) {
        ...
        if (!empty($denumire) && !empty($prescurtare)) {
            try {
                ...
                $successMessage = 'Structura "' . htmlspecialchars($denumire) . '" a fost adăugată cu succes!';
            } catch (PDOException $e) { $errorMessage = 'Eroare la adăugarea structurii. Poate deja există.'; }
        } else {
            $errorMessage = 'Denumirea și prescurtarea sunt obligatorii pentru o structură nouă.';
        }
    }
    
    // --- Procesare Editare Structura ---
    if (isset($_POST['edit_structura'])) {
        ...
        if ($id && !empty($denumire) && !empty($prescurtare)) {
            try {
            ...
            } catch (PDOException $e) { $errorMessage = 'Eroare la actualizarea structurii.'; }
        } else {
            $errorMessage = 'Toate câmpurile sunt obligatorii pentru editare.';
        }
    }

    // --- Procesare Adaugare Substructura ---
    if (isset($_POST['add_substructura'])) {
        ...
        if ($id_struct && !empty($denumire)) {
            try {
                ...
                $successMessage = 'Substructura "' . htmlspecialchars($denumire) . '" a fost adăugată cu succes!';
            } catch (PDOException $e) { $errorMessage = 'Eroare la adăugarea substructurii.'; }
        } else {
            $errorMessage = 'Trebuie să selectați o structură și să completați denumirea pentru o substructură nouă.';
        }
    }
}


// Preluam toate datele necesare pentru afisare (dupa eventuale modificari)
try {
    ...
    $currentTitle = 'Eroare la citire';
    $structuri = [];
    $substructuri = [];
    $errorMessage = 'Eroare la citirea datelor din baza de date.';
}
?>
            ...
            <div class="card-body">
                ...
                <form action="settings.php" method="post">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="dashboard_title" name="dashboard_title" value="<?php echo htmlspecialchars($currentTitle); ?>" required>
                    </div>
        ...
        <div class="card content-card h-100">
            <div class="card-body">
                ...
                <form action="settings.php" method="post" enctype="multipart/form-data">
                    <div class="mb-3">
                        <label for="login_background" class="form-label">Încărcați o imagine nouă</label>
...
<div class="row mt-4">
    <!-- Card Adauga/Editeaza Structura -->
    <div class="col-md-6 mb-4">
        <div class="card content-card h-100">
            <div class="card-body">
                ...
                <div class="table-responsive" style="max-height: 300px;">
                    <table class="table table-sm table-prestige">
    ...
    <!-- Card Adauga/Editeaza Substructura -->
    <div class="col-md-6 mb-4">
        <div class="card content-card h-100">
            <div class="card-body">
...
document.addEventListener('DOMContentLoaded', function() {
    // Script pentru a popula modalul de editare structura
    ...
    if(editStructuraModal) {
        editStructuraModal.addEventListener('show.bs.modal', function (event) {
            ...
            const modalIdInput = editStructuraModal.querySelector('#edit_structura_id');
            const modalDenumireInput = editStructuraModal.querySelector('#edit_structura_denumire');
            const modalPrescurtareInput = editStructuraModal.querySelector('#edit_structura_prescurtare');

            modalIdInput.value = id;
            modalDenumireInput.value = denumire;
            modalPrescurtareInput.value = prescurtare;
        });
    }

    // Script pentru a popula modalul de editare substructura
    ...
    if(editSubstructuraModal) {
        editSubstructuraModal.addEventListener('show.bs.modal', function (event) {
            ...
            editSubstructuraModal.querySelector('#edit_substructura_id_struct').value = id_struct;
